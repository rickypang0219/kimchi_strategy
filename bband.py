...
if __name__ == "__main__":
    ...
    factors_df = factors_df.with_columns(
        [
            (pl.col("upbit_close") / pl.col("binance_close")).alias("factor"),
            (pl.col("binance_close").alias("price")),
        ]
    )
    plt.plot(factors_df["factor"].to_numpy())
    plt.show()
    training_set = factors_df[: len(factors_df) // 2, :]
    backtester = BackTester(training_set)
    rolling_windows = np.array(
        [i for i in range(5, 50, 2)]
        + [i for i in range(100, 501, 20)]
        + [i for i in range(2000, 2500, 20)]
    )
    ...
    forward_set = factors_df[len(factors_df) // 2 :, :]
    forwardtester = BackTester(forward_set)
    rolling_windows = np.array(
        [i for i in range(10, 50, 2)]
        + [i for i in range(100, 501, 20)]
        + [i for i in range(2000, 2500, 20)]
    )
    forwardtester.optimize_params_and_plot_heatmap(rolling_windows, multipliers)
    trade_info = forwardtester._compute_trade_statistics(32, 3.2)
    forwardtester.print_trade_summary_stats(32, 3.2)
    forwardtester.plot_returns(trade_info)
